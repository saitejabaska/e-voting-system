...
app.get(
  ...
  async (request, response) => {
    ...
    if (login_admin_id !== elections.admId) {
...
app.delete(
  ...
  connectEnsureLogin.ensureLoggedIn(),
  async (request, response) => {
    const admId = request.user.id;
    const election = await Election.findByPk(request.params.id);

    if (admId !== election.admId) {
...
app.post(
  ...
  async (request, response) => {
    ...
    const election = await Election.findOne({
      where: { admId: login_admin_id, name: request.body.name },
    });
    if (election) {
...
app.get(
  ...
  async (request, response) => {
    ...
    if (login_admin_id !== election.admId) {
      return response.render("error", {
...
app.get(
  ...
  async (request, response) => {
    ...
    if (request.user && request.user.id && !request.user.voterID) {
      ...
      if (admId !== election.admId && !election.ended) {
      ...
      const admin = await Admin.findByPk(election.admId);
